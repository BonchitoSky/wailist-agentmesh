...
mobile/
  ...
  src/
    ...
    geofence.ts         fence registration + flush on reconnect
    nativeGeofence.ts   bridge to the Android plugin below
    permissions.ts      the disclosure, the ask, and the refusal path
    queue.ts            offline queue for location fixes
  android/app/src/main/java/ai/agentmesh/app/
...
queue. When it is *not* — the common case for a real crossing —
...
**The honest limit:** that makes delivery *late* (next app open) rather than
