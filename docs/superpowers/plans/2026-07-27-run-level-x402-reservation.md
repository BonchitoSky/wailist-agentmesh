...
## Post-implementation amendments (found during review, not in the original plan)

Three real defects surfaced during code review — one during Task 5's task-level review, two during the final whole-branch review — that changed behavior beyond what this plan originally specified. Recorded here so this document stays an accurate account of what was actually built, not just what was proposed.

- **`executeTool402RunLevel` now commits, not releases, once a payment is signed.** The original Task 5 pseudocode released the in-memory pool reservation on any error from `PayTargetFromWallet2`, including a post-signing network failure — understating real spend for money that had already left Wallet 2. Fixed to branch on `Wallet2PayResult.Signed`: release only when signing itself never happened (asset mismatch, over-cap, sign failure); commit in every case where a real payment was sent, network failure or audit-write failure included.
- **A legacy-dialect tool402 attached alongside a v2 tool on the same agent no longer shares the run-level pool.** The original design let both dialects read `X402RelayConfig.Ledger`, which the run-level path swaps to the in-memory v2 pool — so a legacy call's flat fee could spuriously exhaust that pool (hard-blocking the run over a balance the user's real DB credits had nothing to do with) or silently commit against the wrong ledger. Fixed by adding `X402RelayConfig.LegacyLedger` (always the original DB-backed per-call ledger) and `RunFundedToolIDs` (the set of attached tool IDs actually folded into the run-level estimate), so legacy billing and its floor guard are now completely independent of whether the same agent happens to also be run-funded.
- **A `RecordRunFunding` failure after a successful `FundRunReserve` no longer silently falls back to the per-call path.** The original code, on this failure, still returned `funding.ID` — the zero value, `""` — which is the exact sentinel `ExecuteTool402V2` reads as "no run-level funding for this run." That would have routed every subsequent v2 call for the agent onto the old per-call public-relay path, which performs its own full inbound settle per call — Wallet 1 paying twice for the same run, precisely the double-settle bug this whole plan exists to eliminate, triggered by nothing more than a transient DB write failure after real money had already moved on-chain. Fixed to fail the run loudly instead, and to deliberately not release the DB credit reservation in this case (the money is real and sitting in our own Wallet 2 — an operator reconciles it by hand rather than risking a silent double-spend).
- Also closed in the same pass: `reserveAndFundRun` now degrades gracefully (falls back to the unmodified per-call path, reserves nothing) when the platform spend wallet isn't configured, instead of risking a nil-interface panic after credits were already reserved.

See the operator runbook's "Run-level pre-funding" section for the user-facing description of all of the above.
